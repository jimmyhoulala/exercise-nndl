...
class word_embedding(nn.Module):
    def __init__(self,vocab_length , embedding_dim, device):
        super(word_embedding, self).__init__()
        w_embeding_random_intial = np.random.uniform(-1,1,size=(vocab_length ,embedding_dim))
        self.word_embedding = nn.Embedding(vocab_length,embedding_dim).to(device)#使用GPU
...
class RNN_model(nn.Module):
    def __init__(self, batch_sz ,vocab_len ,word_embedding,embedding_dim, lstm_hidden_dim, device):
        ...
        # ???
        self.rnn_lstm = nn.LSTM(input_size=embedding_dim, hidden_size=lstm_hidden_dim, num_layers=2, batch_first=True).to(device)


        ##########################################
        self.fc = nn.Linear(lstm_hidden_dim, vocab_len ).to(device) #使用GPU
        self.apply(weights_init) # call the weights initial function.

        self.softmax = nn.LogSoftmax() # the activation function.
        # self.tanh = nn.Tanh()
    def forward(self,sentence,is_test = False):
        batch_input = self.word_embedding_lookup(sentence).view(1,-1,self.word_embedding_dim).to(self.device)
        ...
        h0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(self.device)
        c0 = torch.zeros(2, batch_input.size(0), self.lstm_dim).to(self.device)

        xavier_init(h0)
        xavier_init(c0)


        output, _ = self.rnn_lstm(batch_input, (h0, c0)) #这里只考虑输出，不考虑Cn和Hn
